...
/**
 * dll_abj_swap - swaps two adjacent nodes of a doubly linked list
 * @list: The doubly linked list of integers to be sorted
 * @left: The node closer to head, right->prev
 * @right: The node closer to tail, left->next
 */
...
void cocktail_sort_list(listint_t **list)
{
	...
	do {
		...
		for (checks = 0; temp->next && checks < shake_range; checks++)
		{
			...
			{
				dll_abj_swap(list, temp, temp->next);
				swapped_f = true;
			}
			else
				temp = temp->next;
		...
		for (checks = 0; temp->prev && checks < shake_range; checks++)
		{
			...
			{
				dll_abj_swap(list, temp->prev, temp);
				swapped_b = true;
			}
			else
				temp = temp->prev;
