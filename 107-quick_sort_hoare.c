...
/**
 * hoare_partision - Hoare partition scheme using rightmost index as pivot;
 ...
 */
int hoare_partision(int *array, size_t low, size_t high, size_t size)
{
	int i, j, pivot, temp;
...
/**
 * hoare_quiksot - recursively sorts array of integers by separating into two
 ...
 * @size: amount of elements in array
 */
void hoare_quiksot(int *array, size_t low, size_t high, size_t size)
{
	size_t border;

	if (low < high)
	{
		border = hoare_partision(array, low, high, size);
		hoare_quiksot(array, low, border, size);
		hoare_quiksot(array, border + 1, high, size);
	}
}
...
void quick_sort_hoare(int *array, size_t size)
{
	...
	hoare_quiksot(array, 0, size - 1, size);
}
